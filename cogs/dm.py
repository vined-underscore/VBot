...
class DMCmds(
  ...
  async def pin(self, ctx, user: discord.User, amount=15, *, msg = None):
    cmsg = ctx.message
    ...
    await delete_msg(cmsg)

  @vbot.command(
    name = "unpin",
    description = "Unpin an amount of messages in a persons DMs"
  )
  async def unpin(self, ctx, user: discord.User, amount=15):
    count = 0
    msg = ctx.message
